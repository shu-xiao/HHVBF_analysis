...
RVec<Int_t> matchJet(Int_t ind1, Int_t ind2, rvec_f GenPruned_pT, rvec_f GenPruned_eta, rvec_f GenPruned_phi, rvec_f GenPruned_mass,Int_t nJet, rvec_f Jet_pt, rvec_f Jet_eta, rvec_f Jet_phi, rvec_f Jet_mass) {
    ...
    for (int i=0;i<nJet;i++) {
        ...
        if (vecJet.DeltaR(vecGen1) < 0.8 && ind[0]<0) ind[0] = i;
        if (vecJet.DeltaR(vecGen2) < 0.8 && ind[1]<0) ind[1] = i;
        if (ind[0]>=0 && ind[1]>=0) {
...
void nMinus1_genMatch() {
    ...
    for (int i=0;i<6;i++) {
        ...
        //t2->AddFriend(t1,"FatJetInfo");
        t2->AddFriend(t1,"t1");
        //pu200List[i]->Add(Form("VBF_HHTo4B_%s_nTuple_PU200.root",parList[i].data());

        RDataFrame d0(*t2);
        cout << "loading RDataframe " << i+1 << endl;

        //auto d = d0.Range(500);
        //auto d = d0.Define("AK4HT","HT(nJet,Jet_pt)").Define("AK8HT","HT(FatJetInfo.nJet,FatJetInfo.Jet_pt)");
        auto d1 = d0.Filter("FatJetInfo.nJet>=2","nJet>=2");
        auto d2 = d1.Define("Hind1","matchInd(nGenPruned,GenPruned_pdgID)[0]").Define("Hind2","matchInd(nGenPruned,GenPruned_pdgID)[1]").Filter("Hind2>=0","2H");
        ...
        histList[0].push_back(*d3.Histo1D({"sdmass1",Form("SDmass1_%s",parList[i].data()),40,0,160},"sdMass1"));
        histList[1].push_back(*d3.Histo1D({"sdmass2",Form("SDmass2_%s",parList[i].data()),40,0,160},"sdMass2"));
        histList[2].push_back(*d3.Histo1D({"AK8mass1",Form("Jetmass1_%s",parList[i].data()),40,0,160},"FatJetMass1"));
        histList[3].push_back(*d3.Histo1D({"AK8mass2",Form("Jetmass2_%s",parList[i].data()),40,0,160},"FatJetMass2"));
        c1->cd(i+1);
        h_ind2D->Draw("colz");
    }
    //c1->Print(pdfname.data());
    ...
    for (int i=0;i<6;i++) {
        c1->cd(i+1);
        AK8jetIDList[i].SetTitle(Form("%s;genmatch AK8 index1;genmatch AK8 index2",parList[i].data()));
